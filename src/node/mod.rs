use crate::{Action, SharedStore, StorageBackend, PocketFlowResult, PocketFlowError};
...
pub trait NodeBackend<S: StorageBackend>: Send + Sync {
    ...
    async fn exec(&mut self, prep_result: Self::PrepResult, context: &ExecutionContext) 
    ...
    async fn post(&mut self, store: &mut SharedStore<S>, prep_result: Self::PrepResult, 
                  exec_result: Self::ExecResult, context: &ExecutionContext) 
    ...
    S: StorageBackend,
{
    ...
    pub async fn run(&mut self, store: &mut SharedStore<S>) -> PocketFlowResult<Action> {
        ...
        let exec_result = self.exec_with_retries(prep_result.clone(), context.clone()).await
            .map_err(|e| PocketFlowError::ExecutionError(format!("Exec failed: {}", e)))?;
        
        // Post phase
        let action = self.backend.post(store, prep_result, exec_result, &context).await
    ...
    async fn exec_with_retries(&mut self, prep_result: B::PrepResult, mut context: ExecutionContext) 
            ...
            match self.backend.exec(prep_result.clone(), &context).await {
                ...
                Err(error) => {
                    if context.can_retry() {
                        ...
                        match self.backend.exec_fallback(prep_result, error, &context).await {
                            Ok(result) => return Ok(result),
                            Err(fallback_error) => {
    ...
    E: Send + Sync + 'static,
{
    ...
    exec_fn: Box<dyn Fn(P, &ExecutionContext) -> Result<E, Box<dyn std::error::Error + Send + Sync>> + Send + Sync>,
    post_fn: Box<dyn Fn(&mut SharedStore<S>, P, E, &ExecutionContext) -> Result<Action, Box<dyn std::error::Error + Send + Sync>> + Send + Sync>,
    max_retries: usize,
    ...
    E: Send + Sync + 'static,
{
    ...
    where
        ...
        ExecFn: Fn(P, &ExecutionContext) -> Result<E, Box<dyn std::error::Error + Send + Sync>> + Send + Sync + 'static,
        PostFn: Fn(&mut SharedStore<S>, P, E, &ExecutionContext) -> Result<Action, Box<dyn std::error::Error + Send + Sync>> + Send + Sync + 'static,
    {
        Self {
            name,
    ...
    E: Send + Sync + 'static,
{
    ...
    async fn exec(&mut self, prep_result: Self::PrepResult, context: &ExecutionContext) 
        -> Result<Self::ExecResult, Self::Error> {
        (self.exec_fn)(prep_result, context)
