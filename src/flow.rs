...
use crate::node::{NodeError, NodeBackend, ExecutionContext};
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            FlowError::NoRouteFound(node_id, action) => {
                write!(f, "No route found from node '{}' for action '{}'", node_id, action)
            }
            ...
            FlowError::InvalidConfiguration(msg) => write!(f, "Invalid flow configuration: {}", msg),
        }
    }
}
...
impl Clone for RouteCondition {
    fn clone(&self) -> Self {
        match self {
            ...
            RouteCondition::KeyEquals(key, value) => RouteCondition::KeyEquals(key.clone(), value.clone()),
        }
    }
}
...
impl Default for FlowConfig {
    fn default() -> Self {
        Self {
            ...
            terminal_actions: vec!["end".to_string(), "complete".to_string(), "finish".to_string()],
        }
    }
}
...
pub trait Flow<S: StorageBackend> {
    ...
    async fn execute(&mut self, store: &mut SharedStore<S>) -> Result<FlowExecutionResult, FlowError>;
    
    /// Execute the flow starting from a specific node
    async fn execute_from(&mut self, store: &mut SharedStore<S>, start_node_id: String) -> Result<FlowExecutionResult, FlowError>;
...
impl<S: StorageBackend + 'static> FlowBuilder<S> {
    ...
    pub fn route(mut self, from: impl Into<String>, action: impl Into<String>, to: impl Into<String>) -> Self {
        ...
        self.routes.entry(from_id).or_insert_with(Vec::new).push(route);
        self
    }
...
impl<S: StorageBackend> BasicFlow<S> {
    ...
    fn find_next_node(&self, current_node_id: &str, action: &Action, store: &SharedStore<S>) -> Result<Option<String>, FlowError> {
        ...
        Err(FlowError::NoRouteFound(current_node_id.to_string(), action_str))
    }
    ...
    S::Error: Send + Sync + 'static,
{
    ...
    fn add_route(&mut self, from_node_id: String, route: Route) -> Result<(), FlowError> {
        self.routes.entry(from_node_id).or_insert_with(Vec::new).push(route);
        Ok(())
    }
    ...
    async fn execute_from(&mut self, store: &mut SharedStore<S>, start_node_id: String) -> Result<FlowExecutionResult, FlowError> {
        ...
        loop {
            ...
            let node = self.nodes.get_mut(&current_node_id)
    ...
    fn validate(&self) -> Result<(), FlowError> {
        ...
        if !self.nodes.contains_key(&self.config.start_node_id) {
            return Err(FlowError::InvalidConfiguration(
                format!("Start node '{}' not found", self.config.start_node_id)
            ));
        }
        ...
        for (from_node, routes) in &self.routes {
            if !self.nodes.contains_key(from_node) {
                return Err(FlowError::InvalidConfiguration(
                    format!("Source node '{}' in routes not found", from_node)
                ));
            }
            ...
            for route in routes {
                if !self.nodes.contains_key(&route.target_node_id) {
                    return Err(FlowError::InvalidConfiguration(
                        format!("Target node '{}' in route not found", route.target_node_id)
                    ));
                }
            }
        }
    ...
    S::Error: Send + Sync + 'static,
{
    ...
    async fn prep(&mut self, _store: &SharedStore<S>, _context: &ExecutionContext) -> Result<Self::PrepResult, Self::Error> {
    ...
    async fn post(
        ...
        let current_depth = context.get_metadata("flow_depth")
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        
        if current_depth > 10 { // Maximum nesting depth
            return Err(FlowError::InvalidConfiguration(
        ...
        store.set("nested_flow_result".to_string(), serde_json::json!({
    ...
    S::Error: Send + Sync + 'static,
{
    ...
    async fn post(
        ...
        store.set(result_key, serde_json::json!({
            "final_action": result.final_action.to_string(),
            "last_node_id": result.last_node_id,
            "steps_executed": result.steps_executed,
            "success": result.success,
...
impl<S: StorageBackend + 'static> FlowBuilder<S> {
    ...
    pub fn build(self) -> BasicFlow<S> {
        ...
        for (from_id, routes) in self.routes {
            for route in routes {
                flow.add_route(from_id.clone(), route).expect("Failed to add route");
            }
        }
...
mod tests {
    use super::*;
    use crate::{InMemoryStorage, LogNode, SetValueNode, Node};
    use serde_json::json;
    ...
    async fn test_basic_flow_creation() {
        ...
        flow.add_node("start".to_string(), Box::new(log_node)).unwrap();
        flow.add_node("end".to_string(), Box::new(set_node)).unwrap();
        
        // Add route
        flow.add_route("start".to_string(), Route {
    ...
    async fn test_flow_builder() {
        ...
        assert_eq!(result.steps_executed, 2);
        assert!(result.success);
        assert_eq!(store.get("builder_result").unwrap().unwrap(), json!("built"));
    }
    ...
    async fn test_conditional_routes() {
        ...
        let mut flow = FlowBuilder::new()
            ...
            .node("fail", fail_node)
            .conditional_route("setup", "check", "success", RouteCondition::KeyEquals("ready".to_string(), json!(true)))
            .conditional_route("setup", "check", "fail", RouteCondition::KeyEquals("ready".to_string(), json!(false)))
            .build();
