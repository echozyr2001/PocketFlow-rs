use pocketflow_rs::{
    Flow, FlowBuilder, FlowNode, Node, NodeBuilder, FunctionNode, Action,
    InMemoryStorage, SharedStore, LogNode, SetValueNode,
};
use serde_json::json;
...
async fn test_nested_flow_basic() {
    ...
    let inner_flow = FlowBuilder::new()
        .start_node("step1")
        .node("step1", Node::new(SetValueNode::new("inner_result".to_string(), json!("inner_done"), Action::simple("complete"))))
        .build();

    // Create outer flow with nested flow
    let mut outer_flow = FlowBuilder::new()
        .start_node("start")
        .node("start", Node::new(SetValueNode::new("outer_start".to_string(), json!("outer_value"), Action::simple("to_nested"))))
        .node("nested", Node::new(FlowNode::new(inner_flow)))
        .node("end", Node::new(SetValueNode::new("outer_end".to_string(), json!("final_value"), Action::simple("done"))))
...
async fn test_deeply_nested_flows() {
    ...
    let level3_flow = FlowBuilder::new()
        .start_node("deep")
        .node("deep", Node::new(SetValueNode::new("level3".to_string(), json!("deep_value"), Action::simple("complete"))))
        .build();

    // Create level 2 flow containing level 3
    let level2_flow = FlowBuilder::new()
        .start_node("mid")
        .node("mid", Node::new(SetValueNode::new("level2".to_string(), json!("mid_value"), Action::simple("to_deep"))))
    ...
    let mut level1_flow = FlowBuilder::new()
        .start_node("start")
        .node("start", Node::new(SetValueNode::new("level1".to_string(), json!("start_value"), Action::simple("to_mid"))))
...
async fn test_nested_flow_error_propagation() {
    ...
    let mut outer_flow = FlowBuilder::new()
        .start_node("start")
        .node("start", Node::new(SetValueNode::new("start".to_string(), json!("start_value"), Action::simple("to_nested"))))
        .node("nested", Node::new(FlowNode::new(inner_flow)))
        .route("start", "to_nested", "nested")
        .build();
