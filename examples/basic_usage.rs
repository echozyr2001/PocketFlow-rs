use pocketflow_rs::{InMemorySharedStore, SharedStore, Action, FileStorage};
use serde_json::json;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    memory_store.set_serializable("llm_config".to_string(), &config).unwrap();
    println!("✅ Stored LLM config");
    
    // Retrieve and deserialize
    if let Some(retrieved_config) = memory_store.get_deserializable::<LLMConfig>("llm_config").unwrap() {
    ...
    file_store.set("persistent_data".to_string(), json!({
    ...
    // Check if keys exist
    println!("🔍 Contains 'temperature': {}", memory_store.contains_key("temperature")?);
    println!("🔍 Contains 'nonexistent': {}", memory_store.contains_key("nonexistent")?);
