...
async fn main() -> PocketFlowResult<()> {
    ...
    store.set("user_input".to_string(), Value::String("Hello, World!".to_string()))?;
    store.set("multiplier".to_string(), Value::Number(serde_json::Number::from(3)))?;
    ...
    let action = log_node.run(&mut store).await
    ...
    let action = transform_node.run(&mut store).await
        .map_err(|e| PocketFlowError::ExecutionError(e.to_string()))?;
    ...
    let action = conditional_node.run(&mut store).await
        .map_err(|e| PocketFlowError::ExecutionError(e.to_string()))?;
    ...
    let action = llm_node.run(&mut store).await
        .map_err(|e| PocketFlowError::ExecutionError(e.to_string()))?;
    ...
    let action = delay_node.run(&mut store).await
        .map_err(|e| PocketFlowError::ExecutionError(e.to_string()))?;
    ...
    let action = set_node.run(&mut store).await
        .map_err(|e| PocketFlowError::ExecutionError(e.to_string()))?;
    ...
    let mut function_node = Node::new(FunctionNode::new(
        ...
        |multiplier: i32, _context: &ExecutionContext| -> Result<i32, Box<dyn std::error::Error + Send + Sync>> {
            Ok(multiplier * 42)
        },
        // Post: store result
        |store: &mut SharedStore<_>, _prep: i32, result: i32, _context: &ExecutionContext| -> Result<Action, Box<dyn std::error::Error + Send + Sync>> {
            store.set("calculation_result".to_string(), Value::Number(serde_json::Number::from(result)))?;
            Ok(Action::simple("calculation_complete"))
        }
    ).with_retries(2));
    
    let action = function_node.run(&mut store).await
        .map_err(|e| PocketFlowError::ExecutionError(e.to_string()))?;
    ...
    for key in ["user_input", "processed_input", "llm_response", "final_result", "calculation_result"] {
